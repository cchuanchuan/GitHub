...
<div class="maincontent">
      <div class="tablename">
      <Strong>员工信息变更</Strong><small>/Table</small>
      </div>
      ...
      if(request.getParameter("no")==null){
    	  ...
    	  sqlperson = "select top "+pagesize+" no,name,sex,phone,birthday,education,entrytime,deptname,positionname from (person inner join dept on person.deptno=dept.deptno) inner join position on person.positionno=position.positionno where person.no='"+request.getParameter("no")+"'";
      }
      ...
      for(int i=0;rsper.next();i++){
    	  ...
    	  %>
    	  <td>
    	    <button><a href="/PerSys/admin/websites/personchange.jsp?no=<%=rsper.getString("no") %>" target="mainframe">编辑</a></button>
    	    <button><a href="/PerSys/admin/websites/personleave.jsp?no=<%=rsper.getString("no") %>" target="mainframe">离职</a></button>
    	  </td>
    	  </tr>
    	  <%
      }
      ...
      <%if(Page!=1){
	      ...
	      <a href="person.jsp?Page=<%=(Page-1)%>">上一页&nbsp;</a>
	      <a href="person.jsp?Page=<%=(Page+1)%>">下一页</a>
    	  <%
      }
      else{
    	  %>
    	  <span>&nbsp;第<%=Page %>页，共<%=totalpage %>页&nbsp;</span>
	      <a href="person.jsp?Page=<%=(Page+1)%>">下一页&nbsp;</a>
	      <a href="person.jsp?Page=<%=(totalpage)%>">最后一页&nbsp;</a>
	      <%
      }
      %>
      </tr>
