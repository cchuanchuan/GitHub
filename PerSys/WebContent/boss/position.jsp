...
<div class="page-container">
  ...
  <div class="menu">
    <ul id="menu" >
      ...
      <li class="focus"><a href="position.jsp"><span>职位信息</span><span class="fr">></span></a>
        <ul class="menusub">
             <%
  ...
  <div class="right-content">
	...
	<div class="maincontent">
      ...
      int totalpage=1;
      int totalrecord=0;
      String sqlpersonnum = "select count(*) from position inner join person on position.positionno=person.positionno";
      ...
      String sqlperson="";
      if(begin!=0){
	      sqlperson = "select top "+end+" position.positionno,positionname,base as 'base pay',name,sex,phone,education,entrytime  from position inner join person on position.positionno=person.positionno order by position.positionno"
	    		  +" except " 
	    		  +"select top "+begin+" position.positionno,positionname,base as 'base pay',name,sex,phone,education,entrytime  from position inner join person on position.positionno=person.positionno order by position.positionno";
      }
      else{
    	  sqlperson = "select top "+pagesize+" position.positionno,positionname,base as 'base pay',name,sex,phone,education,entrytime  from position inner join person on position.positionno=person.positionno order by position.positionno";
      }
      ...
      <%if(Page!=1){
	      ...
	      <a href="position.jsp?Page=<%=(Page-1)%>">上一页&nbsp;</a>
	      <a href="position.jsp?Page=<%=(Page+1)%>">下一页</a>
    	  <%
      }
      else{
    	  %>
    	  <span>&nbsp;第<%=Page %>页，共<%=totalpage %>页&nbsp;</span>
	      <a href="position.jsp?Page=<%=(Page+1)%>">下一页&nbsp;</a>
	      <a href="position.jsp?Page=<%=(totalpage)%>">最后一页&nbsp;</a>
	      <%
      }
      %>
      </tr>
