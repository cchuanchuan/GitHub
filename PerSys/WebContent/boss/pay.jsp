...
<div class="page-container">
  ...
  <div class="menu">
    <ul id="menu" >
      ...
      <li class="focus"><a href="pay.jsp"><span>工资信息</span></a></li>
  ...
  <div class="right-content">
    ...
    <div class="maincontent">
      <div class="tablename">
      <Strong>工资信息</Strong><small>/Table</small>
      </div>
      <%
      if(session.getAttribute("no")!=null){
	      ...
	      <td colspan="99">
	      <form id="dateform" action="pay.jsp">
	        ...
	        String sqlyear="select distinct year from pay where pay.no='"+(String)session.getAttribute("no")+"'";
	        ...
	        <option selected="selected" value=">0">全部</option>
	        <%
	        String sqlmonth = "select distinct month from pay where pay.no='"+(String)session.getAttribute("no")+"'";
	      ...
	      int totalpage=1;
	      int totalrecord=0;
	      String sqlpersonnum = "select count(*)from pay where pay.no='"+(String)session.getAttribute("no")+"'";
	      ...
	      String sqlperson="";
	      if(begin!=0){
		      sqlperson = "select top "+end+" * from pay where no='"+(String)session.getAttribute("no")+"' and year"+year+" and month"+month
		    		  +" except " 
		    		  +"select top "+begin+" * from pay where no='"+(String)session.getAttribute("no")+"' and year"+year+" and month"+month;
	      }
	      else{
	    	  sqlperson = "select top "+pagesize+" * from pay where no='"+(String)session.getAttribute("no")+"' and year"+year+" and month"+month;
	      }
	      ...
	      <%if(Page!=1){
		      ...
		      <a href="pay.jsp?Page=<%=(Page-1)%>">上一页&nbsp;</a>
		      <a href="pay.jsp?Page=<%=(Page+1)%>">下一页</a>
	    	  <%
	      }
	      else{
	    	  %>
	    	  <span>&nbsp;第<%=Page %>页，共<%=totalpage %>页&nbsp;</span>
		      <a href="pay.jsp?Page=<%=(Page+1)%>">下一页&nbsp;</a>
		      <a href="pay.jsp?Page=<%=(totalpage)%>">最后一页&nbsp;</a>
		      <%
	      }
	      %>
	      </tr>
